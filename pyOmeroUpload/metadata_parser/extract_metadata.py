...
def create_acq_metadata_obj(metadata):
    acq_metadata = namedtuple('AcqMetadata', [], verbose=False)

    acq_metadata.channels = get_str_array_val(metadata['channels'])
    acq_metadata.zsections = get_str_array_val(metadata['zsections'])

    acq_metadata.times = get_str_array_val(metadata['time_settings'])
    acq_metadata.positions = get_str_array_val(metadata['positions'])
    # acq_metadata.npumps = metadata['npumps']
    acq_metadata.pump_init = get_str_array_val(metadata['pumpstart'])
...
def find_by_key(data, target):
    for key, value in data.iteritems():
        if isinstance(value, dict):
            for ele in find_by_key(value, target):
                yield ele
        elif key == target:
            yield value
...
def get_dict_by_child_key(data, sibling_key):
    for key, value in data.iteritems():
        if isinstance(value, dict):
            if sibling_key in value.keys():
                yield {key:value}
                # yield value
            else:
                for ele in find_by_key(value, sibling_key):
                    yield ele


# def generate_metadata_sections()
def build_regex_patterns(data):
    global metadata_obj
    metadata_sections = get_dict_by_child_key(metadata_obj, "regex")
...
def handle_table_metadata(s_line, metadata_obj, table_section, attribute):
...
def handle_metadata_section(metadata_section, dict_section,
    ...
    if "data_type" in value_dict.keys():
        if value_dict["data_type"] == "LIST":
            ...
            if "key_label_regex" in value_dict.keys():
...
def handle_attribute_kvp(s_line, metadata_obj, attribute, label_regex,
            ...
            if (len(parts) % 2 == 0 and idx % 2 == 0) or (len(parts) % 2 != 0 and idx == 0):
                ...
                if is_dict_section:
                    ...
                    if label_regex is None:
                        ...
                        for key, value in label_regex.iteritems():
                            rx = re.compile(value)
...
def parse_metadata(schema, filename):
    ...
    build_regex_patterns(metadata_obj)
    ...
    for line in f:
        ...
        if is_table_section:
            table_section = handle_table_metadata(s_line, metadata_obj,
        ...
        metadata_sections = get_dict_by_child_key(metadata_obj, "rx")
        for metadata_section in list(metadata_sections):
            ...
            for value_dict in metadata_section.values():
                ...
                if match:
                    ...
                    print "MATCHCHHHHH!"
                    metadata_section = handle_metadata_section(metadata_section,
        ...
        attribute_kvp = handle_attribute_kvp(s_line, metadata_obj, cur_metadata_attr,
...
def parse_acq_file(filename):
    ...
    parse_metadata(schema, filename)

    acq_metadata = create_acq_metadata_obj(metadata_obj)
    return acq_metadata
...
def extract_log_metadata(filename):
    ...
    log_metadata = create_log_metadata_obj(metadata_obj)
    return log_metadata
...
def main():
    ...
    for exp_file in os.listdir(dir_path):
        if str.lower(exp_file).endswith("acq.txt"):  # the log file containing metadata
            ...
            acq_metadata = parse_acq_file(os.path.join(dir_path, exp_file))
    ...
    for exp_file in os.listdir(dir_path):
        if str.lower(exp_file).endswith("log.txt"):  # the log file containing metadata
            ...
            print dir_path, exp_file
            # log_metadata = extract_log_metadata(os.path.join(dir_path, exp_file))
