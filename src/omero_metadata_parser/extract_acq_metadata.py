...
class AcqMetadataParser(MetadataParser):
    ...
    def update_params(self, line, raw_rx, param_name):
        ...
        idx = list(switch_params_in.keys()).index(param_name)
    ...
    def build_table(self, line, raw_header_rx, raw_row_rx, table_name, df):
        ...
        if len(search_res) > 0:
            ...
            df.loc[len(df)] = list([_f for _f in search_res if _f])
        else:
    ...
    def handle_section(self, line, section_name):
        ...
        # --------------------- times --------------------  #
            ...
            if len(search_res) > 0:
                search_res = list([_f for _f in re.split(rx, line) if _f])

                time_dict = dict(list(zip(time_dict_keys, search_res)))
                return
    ...
    def extract_metadata(self, filename):
        ...
        section_rxs = list(map(re.compile, raw_section_rxs))
